...
public class TaskParserImpl {
    ...
    public static void depthFirst(TaskImpl task){
        ...
        while (!stack.empty()){
            ...
            System.out.println(
                            task0.getTaskName()+
                            " <--"+task0.getParentList().get(0).getTaskName());
    ...
    public static TaskImpl parse(Element element, TaskImpl parent){
        TaskImpl.TaskBuilder taskBuilder = TaskImpl.startBuildTask();
        Optional<String> project = Optional.ofNullable(element.getChildText("project"));
        ...
        taskBuilder
//                .setParentList(Collections.singletonList(parent))
                .setProject(Boolean.parseBoolean(project.orElse("false")))
        ...
        taskListElem.ifPresent(tasksOpt->{
            ...
            tasks.ifPresent(tasksListOpt->{
                List<TaskImpl> childTasks = tasksListOpt.stream()
                        .map(el->parse(el,taskBuilder.build()))
